...
class Metronome:
    ...
    def create_genesis_block(self):
        ...
        return Block(0 , 1, time.time(), "0" * 64, "Genesis Block")
    
    def add_block(self, new_block):
        new_block.blockheight = len(self.chain)
    ...
    def send_empty_block(self):
        ...
        new_block = Block(0,1, time.time(), random_hash, transaction)

        # Add the block to the blockchain
        self.add_block(new_block)

        print(f"Block {len(self.chain) - 1} added to the blockchain:")
...
if __name__ == "__main__":
    ...
    metronome_socket.send(json.dumps(metronome.chain[0].__dict__).encode('utf-8'))
    print("Empty block signal sent to the Blockchain")
